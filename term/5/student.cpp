...
void schedule()
{
	if(current_thread==NULL)
	{
		if(first_ready_queue.size()>0)
		{
			...
			first_ready_queue.pop_front();
			current_thread->clock_times=0;
			current_thread->max_clock_times=g_first_ticks;
		}
		else if(second_ready_queue.size()>0)
		{
			current_thread=second_ready_queue.front();
	...
	{
		if(first_ready_queue.size()>0 && current_thread->max_clock_times>first_ready_queue.front()->max_clock_times)
		{
			current_thread->clock_times=0;
			current_thread->max_clock_times=g_second_ticks;
			second_ready_queue.push_back(current_thread);
...
void on_clock()
{
	...
	if(current_thread->max_clock_times==g_first_ticks && current_thread->clock_times>=g_first_ticks)
	{
	...
	else if(current_thread->max_clock_times==g_second_ticks && current_thread->clock_times>=g_second_ticks)
	{
